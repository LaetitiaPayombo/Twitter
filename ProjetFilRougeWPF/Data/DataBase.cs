...
namespace ProjetFilRouge.Data
{
    class DataBase
    {
        ...
        internal static Utilisateur TrouverUtilisateur(string pseudo, string email)
        {
            SqlConnection connection = Connection;
            Utilisateur u = null;
            SqlCommand command = new SqlCommand("SELECT * FROM utilisateur WHERE pseudo = @Pseudo AND email = @Email", connection);
        ...
        public static bool AjouterCommentaire(Commentaire c)
        {
            ...
            command.Parameters.Add(new SqlParameter("@sujet", c.Comment));
        ...
        internal static Commentaire TrouverCommentaire(string dateCommentaire, string commentaire)
        {
            SqlConnection connection = Connection;
            Commentaire c = null;
            SqlCommand command = new SqlCommand("SELECT * FROM commentaire WHERE dateCommentaire = @dateCommentaire AND commentaire = @commentaire", connection);
            command.Parameters.Add(new SqlParameter("@dateCommentaire", dateCommentaire));
            ...
            while (reader.Read())
            {
                ...
                {
                    Id = reader.GetInt32(0),
                    DateCommentaire = reader.GetString(1),
